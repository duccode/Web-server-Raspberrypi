...
def insert_record( device, dates, times, temp, hum ):
	query = "INSERT INTO `sensordata`(`device`, `datets`, `times`, `temp`, `hum`) VALUES (%s,%s,%s,%s,%s)"
    	args = (device,dates,times,temp,hum)
...
#		temp = temp * 9/5.0 + 32
		...
		insert_record(device,str(date),str(times),format(temp,'.2f'),format(hum,'.2f'))
		time.sleep(10)
